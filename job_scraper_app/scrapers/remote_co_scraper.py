"""
Remote.co Scraper for the Job Scraper Application.

This module implements a scraper for the Remote.co job site.
"""
...
class RemoteCoScraper(BaseScraper):
    ...
    def scrape(self):
        ...
        try:
            ...
            logger.info(f"Scraped a total of {len(job_listings)} job listings from Remote.co")
            
        except Exception as e:
            logger.error(f"Error scraping Remote.co: {e}", exc_info=True)
    ...
    def _extract_company_website(self, text):
        ...
        if urls:
            ...
            excluded_domains = ['remote.co', 'linkedin.com', 'twitter.com', 'facebook.com', 'instagram.com']
            for url in urls:
